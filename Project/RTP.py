...
def pred_fold(MSS_positive, MSS_negative, opts, clf_name):
    ...
    test_pred, test_pred_prob, train_pred, test_labels, all_test_labels = [], [], [], [], []
...
# Main function
def main(gap, pos_sup, neg_sup, clf_name):
    start = time.time()
    
    opts = Options()
    opts.max_g = gap
    opts.sup_pos = pos_sup
    opts.sup_neg = neg_sup
    ...
    accuracy, precision, recall, f_measure, auc = pred_fold(MSS_pos, MSS_neg, opts, clf_name)
