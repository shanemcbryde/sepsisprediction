...
def learn_svm(train_data, train_labels, test_data, test_labels):
	print ("SVM Learning........")
	clf = svm.SVC(probability=True, random_state=2022)
	# param_grid = {"gamma" : [1, 1e-1, 1e-2, 1e-3], "C" : [1, 10, 100]}
	# param_grid = {"gamma" : [0.1], "C" : [1.0], "kernel" : ['linear', 'poly', 'rbf', 'sigmoid']}
	param_grid = {"gamma" : [0.1], "C" : [1.0], "kernel" : ['poly']}
	# If you have a warning about iid, please add 'iid=True' in the following cdoe 
	clf = GridSearchCV(estimator=clf, param_grid=param_grid, cv=5, n_jobs=5)  #### Edit
...
def learn_logistic_regression(train_data,train_labels,test_data,test_labels):
	...
	clf = linear_model.LogisticRegression(penalty='l2', dual=False, tol=0.0001, C=1.0, solver='lbfgs', max_iter=100, random_state=2022)
	clf.fit(train_data, train_labels)
	train_predicted = clf.predict(train_data)
	test_predicted_prob = clf.predict_proba(test_data)
	accuracy = accuracy_score(train_labels, train_predicted)
